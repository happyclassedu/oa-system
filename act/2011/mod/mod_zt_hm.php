...
class mod_zt_hm extends mod_base_info {

    /**
     * mod_load : 模型初始化定义主表等。
     */
    function mod_load() {
        $this->xtb = '#@__hrms_hm_zt_r';
        $this->atb = '#@__hrms_hm_w';
        $this->btb = '#@__hrms_hm_t';
        $this->ctb = '#@__hrms_zt';
        $this->dtb = '#@__hrms_hm_i';
    ...
    function list_read() {
        $table = $this->xtb . ' AS x 
            LEFT JOIN (SELECT * FROM (SELECT * FROM ' . $this->atb . ' ORDER BY oid DESC) AS tmp GROUP BY hm_id) AS y ON y.hm_id = x.hm_id
            LEFT JOIN (SELECT * FROM (SELECT * FROM ' . $this->btb . ' ORDER BY oid DESC) AS tmp GROUP BY hm_id) AS z ON z.hm_id = x.hm_id';
        $field = ' x.id, x.hm_id, x.hm_name, x.hm_code, x.zt_id, x.zt_name, y.post_name, y.post_level, z.org_0_name ';
        $where = ' x.drwx=0 ';
        $order = ' ';
    ...
    function list_read_zt() {
        $arr = $this->xdb->read_all($this->ctb, 'id, name', 'drwx=0');
        $this->print_arr($arr, 1);
        return $arr;
    }
    ...
    function list_read_hm() {
        $arr = $this->xdb->read_all($this->dtb, 'id, name', 'drwx=0 ORDER BY name');
        $this->print_arr($arr, 1);
        return $arr;
    }
}
