...
class mod_lbill extends mod_base_info {

    /**
     * mod_load : 模型初始化定义主表等。
     */
    function mod_load() {
        $this->arr_name = array('转入团关系', '转出团关系', '缴纳团费', '团员身份证明'); //业务名称
    ...
    function list_read() {
        ...
        $where = ' x.drwx=0 AND x.i_type = "团员管理" ' . $str_xyz;
        $order = ' ';
...
//        $arr = $where;
    ...
    function m_get_num($uid, $yname) {
        $table = $this->xtb . ' AS x ';
        if ('' == $uid) {
            $where = ' x.drwx=0 AND x.i_type = "团员管理" AND x.name="' . $yname . '" ';
        } else {
            $where = ' x.drwx=0 AND x.i_type = "团员管理"  AND x.uid=' . $uid . ' AND x.name="' . $yname . '" ';
        }
        $arr = $this->list_num_base($table, $where);
        //        $this->print_arr($arr);
        return $arr;
    ...
    function m_get_sum() {
        $table = $this->xtb . ' AS x ';
        $where = ' x.drwx=0 AND x.i_type = "团员管理" ';
        $arr = $this->list_num_base($table, $where);
        return $arr;
    }
