...
class mod_loh extends mod_base_info {
    ...
    function info_add() {
      ...
      // 查出存档人id
        ...
        if('转入团关系' == $x_arr['name']){
            $this->xdb->update($this->atb, 'league_state="在库"',  'id="' . $x_arr['bid'] . '"');
        } elseif('转出团关系' == $x_arr['name']) {
            $this->xdb->update($this->atb, 'league_state="调出"',  'id="' . $x_arr['bid'] . '"');
        }
