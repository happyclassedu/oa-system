...
class mod_person extends mod_base_info {
    ...
    function mod_load() {
        ...
        $this->key_search = array('loginid', 'name', 'tel', 'email', 'atime');
    }
...
* @return $arr:0（失败）或1（成功）.
    */
    ...
    function  info_init($pid) {
        $arr = array();
        $result_arr = $this->xdb->read_one($this->xtb, '*', 'id="' . $pid . '"');
    ...
    function info_comm() {
        $arr = array();
        $arr['info_time'] = i_time();
        $arr['info_ip'] = $this->get_real_ip();
        $this->print_arr($arr, 1);
        return $arr;
    }
