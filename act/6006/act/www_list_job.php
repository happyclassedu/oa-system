...
if (!$page) {
    ...
    $page['info_num'] = $mod->list_read_job_num($xid);  //该栏目共有内容数
    ...
    include_once 'www_lefter.php';

    $ws['nav'] = $mod->read_nav(0);
    $tmp['name'] = '企业招聘';
    $tmp['url'] = 'list_job_1_1.htm';
    $ws['nav'][] = $tmp;
    ...
    $ws['list'] = $mod->list_read_job($xid, $page['show_num'], $page['show_num']*($page['page_now']-1));
    
    $mkf = m_app . 'www/list_job_' .$xid . '_' .$page['page_now'] . '.htm';  //定义生成文件路径
