...
class mod_menu extends mod_base_info {
    ...
    function list_read($ws_id) {
        $this->list_load();
        $table = $this->xtb . ' AS x ';
        $field = ' id, name, f_name, type, url';
        $where = ' x.drwx in (0, 1, 2, 3) ';
        $order = ' ORDER BY f_id, oid, type DESC ';
     ...
     * @return $arr : 空or数组加密后的字符串.
     */
    function list_read4menu(){
        $arr = i_tree_create_1($this->xtb, '0', ' AND drwx in (0, 1, 2, 3)');
        $this->print_arr($arr, 1);
        return $arr;
    }
