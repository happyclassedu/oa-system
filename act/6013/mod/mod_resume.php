...
class mod_resume extends mod_base_info {
    ...
    function mod_load() {
        ...
        $this->key_search = array('name', 'atime');
    }
...
* @return $arr:0（失败）或1（成功）.
    */
     ...
     * @return $arr:0（失败）或1（成功）.
     */
    function info_email($xid) {
        $email = $this->info_save_load();
        $arr = $this->xdb->update($this->xtb, 'email="' . $email . '"', 'id="' . $xid . '"');
        $this->print_arr($arr);
        return $email;
    }
    ...
    function info_login() {
        ...
        if('' != $info['loginid']) {
            $table = $this->xtb . ' AS x ';
            $field = ' id, loginid, loginpw, name, sex ';
                ...
                if('' != $info['loginpw']) {
                    if($arr_info['loginpw'] == $info['loginpw']) {
                        ...
                        if(isset($_SESSION['login_time']) && time() < ($_SESSION['login_time']+ get_cfg_var('session.gc_maxlifetime'))) {
                            ...
                            $aip = i_real_ip();
                            $this->xdb->update($this->xtb, 'login_time="' . i_time() . '", login_hits="' . $login_hits . '", aip="' . $aip . '"', 'loginid="' . $info['loginid'] . '"');
                            $arr = 'err_longin'; //正常登录
                        } else {
    ...
    function info_name_check($xid) {
        $key = @$_GET['obj_id'];
        $ws_id = @$_GET['ws_id'];
        $info_name = $this->info_save_load();
        $arr = $this->xdb->read_num($this->xtb, ' ' . $key . '="' . $info_name . '" AND id !="' . $xid . '" AND ws_id=' .$ws_id . ' ');
        $this->print_arr($arr);
        return $arr;
    }
