...
class mod_ins_base extends mod_base_info {

    /**
     * mod_load : 模型初始化定义主表等。
     */
    function mod_load() {
        $this->xtb = '#@__cfg_ins_base';
    }
    ...
    function list_load() {
        ...
        $this->key_search = array('year', 'ins_base');
    }
    ...
    function list_read() {
        $this->list_load();
        $table = $this->xtb . ' AS x ';
        $field = ' x.id, x.year, x.ins_base, x.org_rate, x.org_5_rate, x.org_4_rate, x.org_3_rate ';
        $where = ' x.drwx=0 ';
        $order = ' ORDER BY x.year DESC, ins_base ';
