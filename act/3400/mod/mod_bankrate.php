...
class mod_bankrate extends mod_base_info {

    /**
     * mod_load : 模型初始化定义主表等。
     */
    function mod_load() {
        $this->xtb = '#@__cfg_bankrate';
    }
    ...
    function list_load() {
        ...
        $this->key_search = array('year', 'bankrate');
    }
    ...
    function list_read() {
        $this->list_load();
        $table = $this->xtb . ' AS x ';
        $field = ' x.id, x.year, x.bankrate, x.remark ';
        $where = ' x.drwx=0 ';
        $order = ' ORDER BY x.year DESC ';
