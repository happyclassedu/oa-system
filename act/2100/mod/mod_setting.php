...
class mod_setting extends mod_base_info {

    /**
     * mod_load : 模型初始化定义主表等。
     */
    function mod_load() {
        $this->xtb = '#@__czyl_setting';
...
//     /**
//     * list_read_street2community : 根据条件读取list的信息的内容.
//     * @return $arr : 空or数组加密后的字符串.
//     */
//    function  list_read_street2community() {
//        $table = $this->xtb . ' AS x ';
//        $field = ' x.id, x.street, x.street_code ';
//        $where = ' x.drwx=0 AND x.i_type="0" ';
//        $order = ' ORDER BY x.atime DESC ';
//        $arr_info = $this->xdb->read_all($table, $field, $where . $order);
//
//        for($i=0 ; $i<count($arr_info); $i++) {
//            $arr[$i] = $this->xdb->read_one($this->xtb . ' AS x ', ' x.id, x.street, x.street_code ', ' x.drwx=0 AND x.i_type="0" AND x.id=' . $arr_info[$i]['id'] . ' ');
//            $arr[$i]['community'] = $this->xdb->read_all($this->xtb . ' AS x ', ' x.id, x.fid, x.fname, x.community, x.community_code  ', ' x.drwx=0 AND x.i_type=1 AND x.fid=' . $arr_info[$i]['id'] . ' ');
//        }
//        $this->print_arr($arr, 1);
//        return $arr;
//    }
     ...
     /**
     * list_read_street : 根据条件读取list的信息的内容.
     * @return $arr : 空or数组加密后的字符串.
     */
    function  list_read_street() {
        $table = $this->xtb . ' AS x ';
        $field = ' x.id, x.street, x.street_code ';
        $where = ' x.drwx=0 AND x.i_type="0" ';
        $order = ' ORDER BY x.atime DESC ';
    ...
    /**
     * list_read_community : 根据条件读取list的信息的内容.
     * @return $arr : 空or数组加密后的字符串.
     */
    function  list_read_community($xid) {
        $str_where = '';
        if('undefined' != $xid) {
            $str_where = 'AND x.fid=' . $xid;
        } else {
            $str_where = '';
        }
        $table = $this->xtb . ' AS x ';
        $field = ' x.id, x.fid, x.fname, x.community, x.community_code ';
        $where = '  x.drwx=0 AND x.i_type="1" ' . $str_where . ' ';
        $order = ' ORDER BY x.atime DESC ';
        $arr = $this->xdb->read_all($table, $field, $where . $order);
        $this->print_arr($arr, 1);
        return $arr;
