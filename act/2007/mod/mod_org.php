...
class mod_org extends mod_base_info {
    ...
    function list_read() {
        $table = $this->xtb . ' AS x ';
        $field = ' x.id, x.name, x.fid, x.fname, x.intro ';
        $where = ' x.drwx=0 ';
        $order = ' ORDER BY x.fid ';
    ...
    function info_edit($xid) {
        ...
        if ($tmp['name']) {
            $this->xdb->update($this->xtb, ',fname="' . $tmp['name'] . '",', 'fid="' . $xid . '"');
            $this->xdb->update($this->atb, 'org_0_name="' . $tmp['name'] . '"', 'org_0_id="' . $xid . '"');
            $this->xdb->update($this->atb, 'org_1_name="' . $tmp['name'] . '"', 'org_1_id="' . $xid . '"');
        }
        $this->print_arr($arr);
        return $arr;
    }
     ...
     * @return $arr:空or数组加密后的字符串.
     */
    function list_read_fid() {
        $arr = $this->xdb->read_all($this->xtb, 'id, name', 'drwx=0 AND fid=1');
        $this->print_arr($arr, 1);
        return $arr;
    }
}
