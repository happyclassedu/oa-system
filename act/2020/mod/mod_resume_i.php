...
class mod_resume_i extends mod_base_info {

    /**
     * mod_load : 模型初始化定义主表等。
     */
    function mod_load() {
        $this->xtb = '#@__zpw_r_i';  //简历子信息表
    ...
    /**
     * list_num_letter : 根据条件读取list的信息总条数.
     * @return $arr : 空or记录数.
     */
    function list_num_letter() {
        $pid = $_GET['p_id'];
        $type = $_GET['type'];
        $table = $this->xtb . ' AS x ';
        $where = ' x.drwx=0 AND x.p_id="' .$pid .  '" AND x.i_type="' . $type . '"';
