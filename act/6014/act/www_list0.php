...
$ext = ''; //扩展值
if('' != $ws['list']['type_ext'] && isset($ws['list']['type_ext'])) {
     $ext = '0';
}
...
//    $ws['lefter_news'] = $mod->read_lefter_news('52', '5');
    ...
    $ws['page'] = i_list_page($xid, $page, 'list0');

    $ws['news'] = $mod->list_read_news($xid, $page['show_num'], $page['show_num']*($page['page_now']-1));

    $mkf = m_app . 'www/list' . $ext . '_' .$xid . '_' .$page['page_now'] . '.htm';  //定义生成文件路径
