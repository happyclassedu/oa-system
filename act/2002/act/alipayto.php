...
require_once("../inc/10_alipay_cfg.php");
require_once("../mod/lib_alipay_service.php");

/*以下参数是需要通过下单时的订单数据传入进来获得*/
//必填参数
$out_trade_no = $_GET['out_trade_no'];		//请与贵网站订单系统中的唯一订单号匹配
$subject      = $_GET['subject'];//$_POST['aliorder'];	//订单名称，显示在支付宝收银台里的“商品名称”里，显示在支付宝的交易管理的“商品名称”的列表里。
$body         = $_GET['body'];//$_POST['alibody'];	//订单描述、订单详细、订单备注，显示在支付宝收银台里的“商品描述”里
$total_fee    = $_GET['total_fee'];//$_POST['alimoney'];	//订单总金额，显示在支付宝收银台里的“应付总额”里
...
$htm = $alipay->build_form();
echo $htm;
//$parameter = $alipay->parameter;
//$mysign = $alipay->mysign;
//$sign_type = $alipay->sign_type;
//$ali_url = $alipay->build_url();
//$ali_htm = $alipay->build_htm();
////数组
//$arr = array(
//        'url' => $ali_url,
//        'out_trade_no' => $out_trade_no,
