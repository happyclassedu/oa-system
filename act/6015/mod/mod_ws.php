...
class mod_ws extends mod_base_ws {
    ...
    function read_index_news1() {
        $arr = $this->list_read_menu_news('455', '1', '8');
        return $arr;
    }
    ...
    function read_lefter_news() {
        $table = $this->ctb . ' AS x ';
        $field = ' id, name, name_s, atime, img, intro ' . $this->url_info . ' ';
        $where = ' drwx="0" AND ws_id="'. ws_id .'" AND (menu_id="456" OR menu_id="455") ';
        $order = ' ORDER BY atime DESC  LIMIT 0, 8 ';
        $arr = $this->xdb->read_all($table, $field, $where . $order);
        return $arr;
    }
}
