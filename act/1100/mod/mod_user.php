...
class mod_user extends mod_base_info {
    ...
    function list_read() {
        $table = $this->xtb . ' AS x ';
        $field = ' x.id, x.name, x.loginid, x.open, x.intro ';
        $where = ' x.drwx=0 ';
        $order = ' ';
    ...
    function info_add() {
        $this->info_save_load();
        $this->arr['loginpw'] = md5($this->arr['loginpw']);
     ...
     */
    function info_loginid_check($xid) {
        $info_loginid = @$_POST['arr'];
        $info_loginid = i_json2php($info_loginid);
        $arr = $this->xdb->read_num($this->xtb, ' loginid="' . $info_loginid . '" AND id !="' . $xid . '" ');
        $this->print_arr($arr);
        return $arr;
    }
}
