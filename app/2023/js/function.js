 ...
 **/
function m_date_comp(start_date, end_date){
    //开始日期
    var ft_start_date = '';
    if('' == start_date){
        var d = new Date();
        ft_start_date = new Date(d.getYear(),d.getMonth()+1,d.getDate(),d.getHours(),d.getMinutes()).getTime();
    }
    var start = i_obj_val(start_date);
    var arr_start = start.split("-");
    ft_start_date = new Date(arr_start[0],arr_start[1],arr_start[2]).getTime();

    //结束日期
    var ft_end_date ='' ;
    var end =i_obj_val(end_date);
    var arr_end = end.split("-");
    ft_end_date = new Date(arr_end[0],arr_end[1],arr_end[2]).getTime();

    if(ft_start_date <= ft_end_date){
       return true;
    }else {
       return false;
    }
...
/***
 *两个日期天数差的方法 m_date2date_days
 * @param: start_date  （开始日期）
 * @param：end_date （结束日期）
 * @return: 天数.
 ***/

function m_date2date_days(start_date, end_date) {
    var ft_start_date = '';
    if('' == start_date){
        var d = new Date();
        ft_start_date = new Date(d.getYear(),d.getMonth()+1,d.getDate(),d.getHours(),d.getMinutes()).getTime();
    ...
    if('' == end_date){
        var d = new Date();
        ft_end_date = new Date(d.getYear(),d.getMonth()+1,d.getDate(),d.getHours(),d.getMinutes()).getTime();
    } else {
        var arr_end = end_date.split("-");
        ft_end_date = new Date(arr_end[0],arr_end[1],arr_end[2]).getTime();
    }

     var mmsec = (ft_end_date - ft_start_date); //得到时间戳相减 得到以毫秒为单位的差
