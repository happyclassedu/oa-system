...
function m_init_info_read() {
    $.ajax({
        ...
        success : function(txt){
            ...
            if('在库' == m.info['league_state']){
                i_obj_set('d_name', '转出团关系');
            }else if('调出' == m.info['league_state'] || '' == m.info['league_state']){
                i_obj_set('d_name', '转入团关系');
            }
        }
    });
}
...
function m_init_info_ytext_set(){
    var ytext = '';
    ytext +=  '转入团委会名称：' + i_obj_val('d_i_txt0') + '；转出原因：' + i_obj_val('d_i_txt1') +  '；<br>' ;
