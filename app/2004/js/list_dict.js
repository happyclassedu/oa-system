...
function m_list_read_set_plug() {
    ...
    m.xtr.children(':eq(2)').html(m.arr[i]['name_e']);
    m.xtr.children(':eq(3)').html(m.arr[i]['name_s']);
    m.xtr.children(':eq(4)').html(m.arr[i]['data_type']);
    m.xtr.children(':eq(5)').html(m.arr[i]['data_length']);
}