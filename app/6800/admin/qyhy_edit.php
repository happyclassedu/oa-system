...
if(isset($_POST[danny])&&empty($_POST[view_submit])&&!empty($_POST[danny])){
	$sql="SELECT * FROM `{$dbpre}company` WHERE `id`='$_POST[danny]'";
	$query=mysql_query($sql);
	$con=mysql_fetch_array($query);
}
if(isset($_POST[view_submit])){
	if(empty($_POST[danny])&&!empty($_POST[cname])&&!empty($_POST[cadd])&&!empty($_POST[ctel])){
...
VALUES (
NULL , '1', '$_POST[cname]', '$_POST[cadd]', '$_POST[ctel]', '$_POST[cmail]', '$_POST[cweb]', '$_POST[sub_industry]', '$_POST[nature]', '$_POST[cnumber]', '$_POST[capital]', '$_POST[setup]', '$_POST[brief]', '$_POST[products]', '1', '$_POST[province]', '$_POST[city]', '1', '".mktime()."', '$_POST[keyword]', '$_POST[description]', '".mktime()."'
);
";
	$db->query($sql);
	ShowMsg("成功添加一条企业信息！","qyhy_edit.php");
	}else{
		if(!empty($_POST[cname])&&!empty($_POST[cadd])&&!empty($_POST[ctel])){
		$sql="UPDATE `{$dbpre}company` SET `cname`='$_POST[cname]',`brief`='$_POST[brief]',`products`='$_POST[products]',`ctel`='$_POST[ctel]'," .
				"`cadd`='$_POST[cadd]',`cmail`='$_POST[cmail]',`cweb`='$_POST[cweb]',`setup`='$_POST[setup]'," .
				"`capital`='$_POST[capital]',`cnumber`='$_POST[cnumber]',`industry`='$_POST[sub_industry]',`nature`='$_POST[nature]',`city`='$_POST[province]',`local`='$_POST[city]',`keyword`='$_POST[keyword]',`description`='$_POST[description]',`update`='".mktime()."' WHERE `id`=$_POST[danny]";
		$db->query($sql);
		ShowMsg("修改成功","qyhy_edit.php");
		}else{
		ShowMsg("公司名称、公司地址、公司电话不能为空","qyhy_edit.php");
	}
	}
...
<table width="1052" border=0 align=center cellpadding=2 cellspacing=1 bordercolor="#799AE1" class=tableBorder>
            <tbody>
      <tr>
        <th align=center colspan=4 style="height: 23px">陕西企业修改页面</th>
      </tr>
              <tr bgcolor="#DEE5FA" >
                <td width="7%" align="right" valign="center">修改ID：</td>
                <td colspan="3">
				<input type="text" name="danny" value="<?php echo $con[id]; ?>"/><input type="submit" name="idcode" value="确定" />                </td>
              </tr>
              <tr bgcolor="#DEE5FA" >
