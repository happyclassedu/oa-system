...
if(isset($_GET[del])&&!empty($_GET[del])){
	if($_GET[del]=="data"){
		$id=implode(",",$_POST[xm]);
		$sql="DELETE FROM `{$dbpre}view` WHERE `id` in ($id)";
	}else{
        $sql="DELETE FROM `{$dbpre}view` WHERE `id`= '$_GET[del]'";
	}
  $db->query($sql);
  ShowMsg("成功删除景点信息。","-1");
...
<form name="form" method="post" action="">
  <table width="1052" border=0 align=center cellpadding=2 cellspacing=1 bordercolor="#799AE1" class=tableBorder>
    <tbody>
      <tr>
        <th align=center colspan=7 style="height: 23px">陕西景点列表</th>
      </tr>
      <tr bgcolor="#DEE5FA">
        <td colspan="7" align="left" class=txlrow><span style="float:left">类别筛选：</span>
      ...
      <tr align="center" bgcolor="#799AE1">
        ...
        <td width="53%" align="left" class="txlHeaderBackgroundAlternate">景点名称</td>
        <td width="8%" align="center" class="txlHeaderBackgroundAlternate">所在城市</td>
        <td width="10%" align="center" class="txlHeaderBackgroundAlternate">所在地区</td>
        <td width="8%" align="center" class="txlHeaderBackgroundAlternate">查看资数</td>
...
$counts=mysql_num_rows(mysql_query("SELECT * FROM `xm_view` $catlimit"));
$getpageinfo=page($page,$counts,25);
$sql="SELECT * FROM `{$dbpre}view` $catlimit $getpageinfo[sqllimit]";
$query=$db->query($sql);
while($row=mysql_fetch_array($query)){
?>
      <tr bgcolor="#DEE5FA">
        <td align="center" class="txlrow"><?php echo $row[id]; ?></td>
        <td align="left" class="txlrow"><?php echo $row[vname]; ?></td>
        <td align="center" class="txlrow"><?php echo $city[$row[vcity]]; ?></td>
        <td align="center" class="txlrow"><?php echo $county[$row[vcounty]]; ?></td>
        <td align="center" class="txlrow"><?php echo $row[vclick]; ?></td>
        <td align="center" class=txlrow><a href="add_view.php?edit=<?php echo $row[id]; ?>">编辑</a> <a href="?del=<?php echo $row[id]; ?>">删除</a></td>
...
?>
      <tr bgcolor="#DEE5FA">
        ...
        <td colspan="5" align="center" class=txlrow><?php echo $getpageinfo['pagecode'];?></td>
