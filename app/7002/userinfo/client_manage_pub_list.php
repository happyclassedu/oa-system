           ...
           default:
           	   ...
           	   break;         
						...
						{ 
								...
								if ($clientname<>"")
								$gqquery ="SELECT  *  from client where clientname like '%".$clientname."%' and suoyq='公共' ORDER BY id desc LIMIT ".$nowpage.","."$dbzz_bianma_pagenum"; 
								else
								$gqquery ="SELECT  *  from client where suoyq='公共' ORDER BY id desc LIMIT ".$nowpage.","."$dbzz_bianma_pagenum"; 
					...
					if ($clientname<>"")
					$query0 ="SELECT id from client WHERE clientname like '%".$clientname."%' and suoyq='公共'";
					else
	  			$query0 ="SELECT id from client where suoyq='公共'";
...
<form name="myform" method="post" action="client_manage_pub_list.php" onsubmit="return FormCheck();">
...
<table border="0" align="center" cellpadding="0" cellspacing="0" bgcolor="#CCCCCC" class="bianmamanage">
  <tr> 
    <td bgcolor="#EBEBEB" class="tishi" colspan="7">&nbsp;公共客户信息</td>
  </tr>
...
if ($ggallrows==0)
{
	echo "<tr><td align='center' height='25' bgcolor='#FFFFFF' colspan='7'>暂无您要查询的数据！</td></tr>";
				...
				<td  width=25%  class="usermanage_2" nowrap><a href="client_detail.php?id=<?=$id;?>" target="_blank" title="查看[<?=$str_clientname;?>]详细信息！"><?=$str_clientname;?></a></td>
				...
				<td  width=8%  style="padding:1px;height:20px;line-height:20px;border-left:1px solid #D8D8D8;border-bottom:1px solid #D8D8D8;">&nbsp;
					<?
//					if ($adduser==trim($_SESSION["userdlname"]) or "admin"==trim($_SESSION["userdlname"])) echo "<a href='client_edit_list.php?id=".$id."' title=编辑此数据>编辑</a> <a href='client_del.php?id=".$id."' onclick='return ConfirmDel();' title='如客户下存在联系人，则不允许删除。'>删除</a>";
...
?>	
	...
	$link="client_manage_pub_list.php?clientname=".$clientname;
