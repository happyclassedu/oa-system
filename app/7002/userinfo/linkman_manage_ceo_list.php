						...
						{ 
								...
								switch ($select_class) {
								    case "企业名称":
												if ($clientname<>"")
												{
													$gqquery ="SELECT  *  from linkname where   clientname like '%".$clientname."%' ORDER BY id desc LIMIT ".$nowpage.","."$dbzz_bianma_pagenum";
													}
													else
													{
														$gqquery ="SELECT  *  from linkname  ORDER BY id desc LIMIT ".$nowpage.","."$dbzz_bianma_pagenum"; 
														}
								        break;
								    case "联系人":
												if ($clientname<>"")
												{
													$gqquery ="SELECT  *  from linkname where  name like '%".$clientname."%' ORDER BY id desc LIMIT ".$nowpage.","."$dbzz_bianma_pagenum";
													}
													else
													{
														$gqquery ="SELECT  *  from linkname  ORDER BY id desc LIMIT ".$nowpage.","."$dbzz_bianma_pagenum"; 
							...
							if($page<=0) 
								...
								switch ($select_class) {
								    case "企业名称":
												if ($clientname<>"")
												{	
													$query0 ="SELECT id from linkname where clientname like '%".$clientname."%' ";
													}
													else
													{
														$query0 ="SELECT id from linkname ";
														}
								        break;
								    case "联系人":
												if ($clientname<>"")
												{	
													$query0 ="SELECT id from linkname where name like '%".$clientname."%'";
													}
													else
													{
														$query0 ="SELECT id from linkname";
...
<table  border="0" class="daohang" cellspacing="0" cellpadding="0">
  <tr>
    <td>&nbsp;客户资源管理->领导查询联系人</td>
  </tr>
</table>

<form name="myform" method="post" action="linkman_manage_ceo_list.php" onsubmit="return FormCheck();" style="padding:0px;margin:0px 0px 0px 0px;">	
...
?>	
	...
	$link="linkman_manage_ceo_list.php?clientname=".rawurlencode($clientname)."&select_class=".rawurlencode($select_class);
