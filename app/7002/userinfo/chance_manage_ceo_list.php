						...
						{ 
								...
								switch ($select_class) {
								    case "企业名称":
												if ($clientname<>"")
												{
													$gqquery ="SELECT  *  from chance where and clientname like '%".$clientname."%' ORDER BY id desc LIMIT ".$nowpage.","."$dbzz_bianma_pagenum";
													}
													else
													{
...
<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" class="bianmamanage">
  <tr> 
			...
			<select name=select_class size=1 id="select_class" class="crm_select">
                ...
                <OPTION value="商机名称"  <?if ($select_class=="商机名称") echo "selected";?>>-商机名称-</OPTION>
...
?>	
	...
	$link="chance_manage_ceo_list.php?clientname=".rawurlencode($clientname)."&select_class=".rawurlencode($select_class);
