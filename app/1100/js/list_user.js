...
function m_list_read_set_plug() {
    ...
    if ('open' == m.arr[i]['open']) {
        m.arr[i]['open'] = '正常';
    } else if ('auditing' == m.arr[i]['open']) {
        m.arr[i]['open'] = '审核';
    } else if ('close' == m.arr[i]['open'] || 'leave' == m.arr[i]['open']) {
        m.arr[i]['open'] = '禁用';
    }

    m.xtr.children(':eq(3)').html(m.arr[i]['open']);
    m.xtr.children(':eq(4)').html(m.arr[i]['intro']);
}