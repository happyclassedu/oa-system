...
function m_act_url_plug() {
    i_mdi_open('./info_person.htm?a=ucenter&x=' + m.xid, '个人会员中心', 1);
    return false;  //可以终止跳转
}
...
function m_info_save_plug() {
    ...
    if(m.tmp == '' && (m.tmp).length == 0){
        ...
        if (!m_isPasswd(m.tmp)) {
            m_error_msg('d_error_loginpw', '密码由6-20个字符（字母、数字、符号）组成！', '0');
            return false;
        }else {
    ...
    if(m.tmp == '' || (m.tmp).length == 0){
        m_error_msg('d_error_mobile', '手机号码不能为空，请填写！',  '0');
        return false;
    } else {
        if (true == i_verify_mobile(m.tmp)) {
            m_error_msg('d_error_mobile', '通过', '2');
        } else {
            m_error_msg('d_error_mobile', '手机号码格式不正确，请重新填写！',  '0');
            return false;
        }
    }
