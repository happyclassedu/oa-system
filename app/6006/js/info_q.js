...
function m_info_save_plug() {
    ...
    if ('' == m.tmp || (m.tmp).length == 0) {
        m_error_msg('d_error_name', '留言标题不能为空，请填写！', '0');
        return false;
    } else {
    ...
    if ('' == m.tmp || (m.tmp).length == 0) {
        m_error_msg('d_error_q_name', '留言者不能为空，请填写！', '0');
        return false;
    } else {
        m_error_msg('d_error_q_name', '通过', '2');
    }
    ...
    if ('' == m.tmp || (m.tmp).length == 0) {
        m_error_msg('d_error_q_intro', '留言内容不能为空，请填写！', '0');
        return false;
    } else {
        m_error_msg('d_error_q_intro', '通过', '2');
    }
