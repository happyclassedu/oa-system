...
function m_btn_load_plug() {
    ...
    $('#btn_reset').click(function(){
        ...
        i_obj_set('d_nation', '');
        i_obj_set('d_native', '');
        i_obj_set('d_party_branch', '');
        i_obj_set('d_jparty_time', '');
        i_obj_set('d_univ', '');
        i_obj_set('d_cname', '');
        i_obj_set('d_tel0', '');
        i_obj_set('d_tel1', '');
...
function m_info_save_plug() {
    ...
    if(m.tmp == '' || (m.tmp).length == 0){
        i_error_msg('d_error_idcard', '身份证号码不能为空，请填写！',  '0');
        return false;
    } else {
        i_check_cardid('d_idcard', 'd_error_idcard', 'd_sex' , 'd_birth')
    }
    ...
    if ('' == m.tmp || (m.tmp).length == 0) {
        i_error_msg('d_error_birth', '出生日期不能为空，请填写！', '0');
        return false;
    } else {
        i_error_msg('d_error_birth', '通过', '2');
    }

    m.tmp = i_obj_val('d_cname');
    if ('' == m.tmp || (m.tmp).length == 0) {
        i_error_msg('d_error_cname', '工作单位不能为空，请填写！', '0');
        return false;
    } else {
        i_error_msg('d_error_cname', '通过', '2');
    }
    ...
    $.ajax({
        ...
        success : function(txt){
            if('1'== txt){
               ...
               i_error_msg('d_error_loginid', '恭喜你，用户名' + i_json2js(arr) + '可以使用！',  '2');
            } 
        }
    });
}
