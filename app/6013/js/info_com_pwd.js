    ...
    $('#btn_edit_pwd').click(function(){
       if(m_info_validate_plug()){
            ...
            if('' != loginpw_x && '' != loginpw_qr && loginpw_x==loginpw_qr){
                ...
                i_error_msg('d_error_loginpw_qr', '确认新密码与新密码不一致，请填写！', '0');
                return false;
            }
        }
...
function m_info_validate_plug(){
    ...
    if ('' == m.tmp) {
        i_error_msg('d_error_loginpw_x', '新密码不能为空，请填写！', '0');
        return false;
    } else {
        i_error_msg('d_error_loginpw_x', '通过', '2');
    }

    m.tmp = i_obj_val('d_loginpw_qr');
    if ('' == m.tmp) {
        i_error_msg('d_error_loginpw_qr', '确认新密码不能为空，请填写！', '0');
        return false;
    } else {
        i_error_msg('d_error_loginpw_qr', '通过', '2');
    }
...
function m_error_init(){
    i_error_msg('d_error_loginpw_x', '可用英文字母、数字，6-20位！', '1');
    i_error_msg('d_error_loginpw_qr', '可用英文字母、数字，6-20位！', '1');
}
