...
//    i_mdi_open('./info_resume_itskill.htm?a=add&x=' + rid +'&rid='+ rid, '简历--IT技能', 1);
    return false;  //可以终止跳转
}
...
function m_info_save_plug() {
    ...
    if(m.tmp == '' && (m.tmp).length == 0){
        m_error_msg('d_error_name', '培训机构不能为空，请填写！',  '0');
        return false;
    } else {
    ...
    if(m.tmp == '' && (m.tmp).length == 0){
        m_error_msg('d_error_i_txt0', '课程名称不能为空，请填写！',  '0');
        return false;
    } else {
...
function m_info_num() {
    $.ajax({
        url : i_act + 'a=list_num&rid=' + rid + '&type=train',
...
m_list_read = function(show_num, page_now) {
    $.ajax({
        url : i_act + 'a=list_read&show_num=' + show_num + '&page_now=' + page_now + '&rid=' + rid + '&type=train',
...
function m_read_list_btn() {
    $('.btn_edit').click(function(){
        ...
        xid = m.arr[xid]['id'];
        i_mdi_open('./info_resume_train.htm?a=edit&x=' + xid + '&rid=' + rid,'英才网--教育经历', 1);
    });
