...
//    i_mdi_open('./info_resume_item.htm?a=add&x=' + rid +'&rid='+ rid, '简历--项目经验', 1);
    return false;  //可以终止跳转
}
...
function m_info_save_plug() {
    ...
    if(m.tmp == '' && (m.tmp).length == 0){
        m_error_msg('d_error_i_txt0', '技能类型不能为空，请填写！',  '0');
        return false;
    } else {
    ...
    if(m.tmp == '' && (m.tmp).length == 0){
        m_error_msg('d_error_name', '技能名称不能为空，请填写！',  '0');
        return false;
    } else {
    ...
    if(m.tmp == '' && (m.tmp).length == 0){
        m_error_msg('d_error_i_txt1', '使用时间不能为空，请填写！',  '0');
        return false;
    } else {
    ...
    if(m.tmp == '' && (m.tmp).length == 0){
        m_error_msg('d_error_i_txt2', '掌握程度不能为空，请填写！',  '0');
        return false;
    } else {
...
function m_info_num() {
    $.ajax({
        url : i_act + 'a=list_num&rid=' + rid + '&type=itskill',
...
m_list_read = function(show_num, page_now) {
    $.ajax({
        url : i_act + 'a=list_read&show_num=' + show_num + '&page_now=' + page_now + '&rid=' + rid + '&type=itskill',
...
function m_read_list_val() {
    ...
    for(var i=0 ; i<m.arr.length; i++) {
        ...
        m.xtr.children(':eq(1)').html(m.arr[i]['i_txt0']);
...
function m_read_list_btn() {
    $('.btn_edit').click(function(){
        ...
        xid = m.arr[xid]['id'];
        i_mdi_open('./info_resume_itskill.htm?a=edit&x=' + xid + '&rid=' + rid,'英才网--专业技能', 1);
    });
