...
function m_list_job_val() {
    ...
    for(var i=0 ; i<m.arr.length; i++) {
        ...
        m.xtr.children(':eq(5)').html('<a href="./info_job_look.htm?a=view&x=' + m.arr[i]['id'] + '&cid=' + m.arr[i]['cid'] + '" class="uline">预览</a>');
...
function m_list_resume_val() {
    ...
    for(var i=0 ; i<m.arr.length; i++) {
        ...
        m.atr.children(':eq(4)').html('<a href="./info_resume_look.htm?a=view&x=' + m.arr[i]['id'] + '" class="uline">预览</a>');
        m.tmp += m.atr.parents().html();
    }
    $('#list_resume tbody').html(m.tmp);
}

//function m_list_resume_btn() {
//    $('.btn_resume_look').click(function(){
//        var xid = this.parentNode.parentNode.id;
//        m.tmp = m.arr[xid]['id'];
//        i_mdi_open('../htm/info_resume_look.htm?a=view&x=' + m.tmp);
//    });
//}
