...
function m_info_add_plug() {
    ...
    i_obj_set('d_i_state0', '正常');
    i_obj_set('d_i_type', '1'); //1代表续保缴费
...
function m_init() {
    $.ajax({
        url : g.act + 'list_acc.php?a=list_read&i_type=2',
        success : function(txt){
...
function m_info_acc_plug(){
    $('#d_acc_level').html('');  //清空tbody里面的内容
    var option_txt = '';
    option_txt += '<option value="">请选择</option>';
    for(var i=0 ; i<m.arr_info.length; i++) {
        option_txt += '<option value="'+ m.arr_info[i]['acc_level'] +'">'+ m.arr_info[i]['acc_level'] + '</option>';
    }
    $('#d_acc_level').append(option_txt);

    $('#d_acc_level').change( function() {
        var i =  $("#d_acc_level").get(0).selectedIndex - 1;
        i_obj_set('d_acc_fee', m.arr_info[i]['acc_level']);
        i_obj_set('d_finance_fee', m.arr_info[i]['finance_fee']);
    });
}
