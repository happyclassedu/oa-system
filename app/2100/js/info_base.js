...
//    i_obj_disable('d_i_state0');
    i_obj_set('d_i_type', '4'); //3代表年度银行利率
...
function m_info_save_plug() {
    ...
    m.tmp = i_obj_val('d_base_fee');
    if ('' == m.tmp) {
         alert('基础养老金标准不能为空，请填写！');
        return false;
    }

     m.tmp = i_obj_val('d_increase_fee');
    if ('' == m.tmp) {
         alert('基础养老金增加数不能为空，请填写！');
        return false;
    }
...
function m_info_name_check(obj_id) {
    ...
    if ('' != m.arr) {
        $.ajax({
            url : i_act + 'a=info_name_check&i_type=4&x=' + m.xid + '&obj_id=' + obj_id.substr(2),
