...
//====定义此类私有变量==========
    ...
    private $cfg_key = array('xtb','f_id','sql');  //传入参数字段
...
//        print_r($this->arr_ok);
    ...
    function tree_scan() {
        ...
        $this->sql_tmp = " f_id = '$this->f_id' $this->sql";
...
//        echo '<hr>';
        ...
        for($i=0; $i<count($arr); $i++) {
            ...
            $this->f_id = $arr[$i]['id'];
            $this->arr[$f_id][$i]['layer'] = $this->layer_now-1;
            $this->arr[$f_id][$i]['fix'] = $this->fix_show();
            $this->tree_scan();
        }
        $this->layer_now--;
    }
